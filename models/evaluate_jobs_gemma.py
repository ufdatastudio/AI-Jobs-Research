...
def build_messages(prompt_text: str):
    return [
        {
            "role": "system",
            "content": [{"type": "text", "text": "You are an expert evaluator of AI-pedagogy job postings. Always return valid JSON only."}],
        },
        {
            "role": "user",
...
def evaluate_job_postings(csv_path: str,
                          output_dir: str = "results/JobPostings/Gemma",
                          processor=None,
                          model=None,
                          max_new_tokens: int = 512,
                          temperature: float = 0.2,
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate job postings for AI-pedagogy alignment using Gemma 3 as a judge.")
    parser.add_argument("--csv_path", type=str, required=True, help="Path to extracted job fields CSV file.")
    parser.add_argument("--output_dir", type=str, default="results/JobPostings/Gemma", help="Directory to save outputs.")
    ...
    evaluate_job_postings(args.csv_path, args.output_dir,
                          processor, model, args.max_new_tokens, args.temperature, args.few_shot)
    print("\nEvaluation complete.")
