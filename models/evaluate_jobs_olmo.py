...
# Default to instruction-tuned variant for better adherence to JSON prompting
...
def evaluate_job_postings(csv_path: str,
                          output_dir: str = "results/JobPostings/OLMo",
                          tokenizer=None,
                          model=None,
                          max_new_tokens: int = 512,
                          temperature: float = 0.2,
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate job postings for AI-pedagogy alignment using OLMo as a judge.")
    parser.add_argument("--csv_path", type=str, required=True, help="Path to extracted job fields CSV file.")
    parser.add_argument("--output_dir", type=str, default="results/JobPostings/OLMo", help="Directory to save outputs.")
